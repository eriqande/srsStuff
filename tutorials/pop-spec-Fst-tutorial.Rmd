...
the srsStuff package to compute population-specific Fst from low-coverage
whole genome sequencing data. This package uses
...
These topics are covered in the next two sessions.
...
is used to write it out to a simple flat text file for use.
...
downloaded them one at ta time to your `~/Downloads` directory.
...
````
Once that is done, _in this case, be sure to_ `gunzip two-small-chromos-flat.txt.gz`.
...
which population we are putting it into.  Here is what that looks like:
```{r}
meta
```
...
to allow the resident and migratory populations from each population share an
...
Here we do that:
```{r}
treelist <- split(x = meta$indiv, f = meta$pop)
```
...
we everything has been grouped into the Root node, like this:
```{r}
...
```{r}
set.seed(1234) # tree layout in the plot is a random, and this value works well
...
If you want to see it is a more traditional dendrogram, you can access the
...
Now we just feed `input_for_srs` and the flat text file of allele depths into
...
On the example data, this takes about 10 seconds.
```{r}
